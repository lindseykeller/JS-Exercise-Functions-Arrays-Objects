...
makePersonObject(29, "Lindsey", "lhenager09@gmail.com");
/**
 * ### Challenge `getName`
 * 
 * @instructions
...
*/
const user = {id: 1, name: "Leia", email: "leia@leia.com"};
...
function getCarInfoByIndex(inventoryArr, index) {

    const carInfo = inventoryArr[index]
...
*/
function getLastCarInfo(carsArr) {

  /* code here */
  return `The car is a ${carsArr[carsArr.length -1].car_make} ${carsArr[carsArr.length -1].car_model}`; 
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(carsArr, idNum) {
  /* code here */
  return `The car is a ${carsArr.idNum.car_make} ${carsArr.idNum.car_model}`; 
}
...
*/
function getModelYears(carInvArr) {
  /* code here */
  let carYearsArr = [];
  for(let i = 0; i < carInvArr.length; i++ ) {
   carYearsArr.push(carInvArr[i].car_year);
  }
  return carYearsArr;
}
...
*/
function getOlderCars(carsInvArr, yearNum ) {
 const oldCarsArr = [];
 for (let i = 0; i < sortCarInventory.length; i ++){
   if (carsInvArr[i].car_year < yearNum) {
     oldCarsArr.push(carsInvArr[i].car_year);
   }
 }
  /* code here */
  return oldCarsArr;
 ...
 * in the same order as they appear in the original inventory.
*/
function getGermanCars(carsInvArr) {
  const germanCars = [];
  /* code here */
  for (let i = 0; i < carsInvArr.length; i++) {
      if (carsInvArr[i].car_make === "Audi" || carsInvArr[i].car_make === "Mercedes-Benz" || carsInvArr[i].car_make === "Volkswagen" || carsInvArr[i].car_make === "BMW" )
      {
        germanCars.push(carsInvArr[i]);
      }
  }
  return germanCars;
}
...
function carMaker(num) {
  /* code here */
  let carMiles;
  const carMilesTracker = {
    odometer: num,
    drive: function drive (num) {
     carMiles.odometer = carMilesTracker.odometer + num;
    }
  }
  return carMiles;
}
